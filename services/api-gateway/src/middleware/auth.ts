    ...
    try {
      ...
      if (apiKeyHeader && !options.jwtOnly) {
        ...
        if (result.success) {
          ...
          authenticated = true;
          logger.debug(
            `API Key authentication successful: ${result.apiKey?.name}`,
            {
      ...
      if (!authenticated && authHeader && !options.apiKeyOnly) {
        if (authHeader.startsWith("Bearer ")) {
          ...
          if (result.success) {
            ...
            authenticated = true;
            logger.debug(
              `JWT authentication successful: ${result.user?.email}`,
              {
                requestId: req.requestId,
      ...
      if (!authenticated && !options.optional) {
        ...
        return res.status(HTTP_STATUS_CODES.UNAUTHORIZED).json({
          ...
          error: {
            ...
            message:
              "Authentication required. Provide valid Authorization header or X-API-Key",
          },
          metadata: {
            requestId: req.requestId,
            timestamp: new Date(),
...
export function createTestTokens() {
  if (config.environment !== "development") {
    throw new Error(
      "Test tokens can only be created in development environment",
    );
  }
