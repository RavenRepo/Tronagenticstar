import { ChiefArchitect, FrameworkRouter, AgentFactory, TaskType } from "./index.js";
...
export class AgentForgeOrchestrator {
     ...
     */
    async processTask(taskType, parameters) {
        const task = {
     ...
     */
    async queryContext(query, taskType) {
    ...
    async createDefaultAgents() {
        ...
        await this.chiefArchitect.registerAgent('architecture-agent-001', 'architecture', TaskType.ARCHITECTURE, [
        ...
        await this.chiefArchitect.registerAgent('security-agent-001', 'security', TaskType.SECURITY, [
            'security_scan',
            'policy_check',
        ...
        await this.chiefArchitect.registerAgent('quality-agent-001', 'quality', TaskType.QUALITY, [
            'code_quality_check',
            'run_tests',
            'analyze_coverage',
...
export async function runDemo() {
    ...
    try {
        ...
        const context = await orchestrator.queryContext("architecture patterns microservices", TaskType.ARCHITECTURE);
