...
if OPENAI_API_KEY:
    ...
    logger.warning("OPENAI_API_KEY not set. AI capabilities will be limited.")
...
class PythonExpertAgent:
    ...
    async def store_memory(self, memory_entry: MemoryEntry):
        ...
        try:
            ...
            # Store in Qdrant
            point = PointStruct(
                id=memory_entry.entry_id,
                vector=embedding,
                payload=memory_entry.dict()
            )
    ...
    async def generate_python_code(self, task: PythonTask) -> TaskResult:
        ...
        knowledge_context = "\n".join([k.get("content", "") for k in relevant_knowledge])
...
Generate clean, efficient, well-documented Python code that follows best practices."""
        ...
        try:
            if OPENAI_API_KEY:
                ...
                # Fallback without AI
                generated_code = f"# Python code for: {task.parameters.prompt}\n# AI not available - placeholder implementation\npass"
                confidence = 0.3
            ...
            memory = MemoryEntry(
                ...
                lessons_learned=["Applied semantic knowledge search", "Integrated style guidelines"],
                patterns_identified=["Code generation pattern", "Knowledge integration pattern"]
            )
    ...
    async def review_python_code(self, task: PythonTask) -> TaskResult:
        ...
        review_knowledge = await self.search_knowledge(f"python code review {' '.join(task.parameters.focus_areas)}")
    ...
    def get_knowledge_base_size(self) -> int:
        ...
        try:
            collection_info = self.qdrant_client.get_collection(self.knowledge_collection)
...
async def search_knowledge(query: str, limit: int = 5, _: bool = Depends(verify_orchestrator)):
...
async def add_knowledge(knowledge: PythonKnowledgeBase, _: bool = Depends(verify_orchestrator)):
