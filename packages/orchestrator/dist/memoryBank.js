...
export class MemoryBankManager extends EventEmitter {
    constructor(retrieverServiceUrl) {
        super();
     ...
     */
    getAgentMemories(agentId, limit) {
     ...
     */
    getSharedContext(taskType, limit = 50) {
        const allMemories = [];
        ...
        const filtered = taskType
            ? allMemories.filter(memory => memory.tags.includes(taskType) ||
     ...
     */
    async performCleanup(maxAgeMs = 24 * 60 * 60 * 1000) {
